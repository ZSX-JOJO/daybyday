...
m		执行多行匹配。
				...
				方括号:
/**
方括号用于查找某个范围内 [纵向模糊匹配] 的字符   方括号也可以称之为 字符组(字符类)
*/
表达式	描述
...
//栗子:	goodGirl  使用/good|goodGirl/匹配时,结果是good,多选分支也是惰性的
...
*/
元字符	描述
[^]或[\w\W]或[\d\D]或[\s\S] //匹配任意字符 

.	查找单个字符，除了换行和行结束符。//[^\n\r\u2028\u2029]通配符 除 换行符 回车符 行分隔符 段分隔符 之外的其他任何字符(想一下省略号...中的每个点,)
\w	查找单词字符。//[0-9a-zA-Z_]表示 数字 大小写字母和下划线,也称单词字符.(w即word 如此记忆)
\W	查找非单词字符。//非单词字符
...
\uxxxx	查找以十六进制数 xxxx 规定的 Unicode 字符。
				...
				量词:
//又被叫做 重复  下面的都是系统自带的一些简写形式   [横向模糊匹配]
